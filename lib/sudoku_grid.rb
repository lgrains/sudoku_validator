class SudokuGrid
  ...
  def grid(grid_number)
    row_set = numbers_for_grid(grid_number, 'row')
    col_set = numbers_for_grid(grid_number, 'column')
  ...
  def numbers_for_grid(grid_number, type)
    sets = [[0,1,2],[3,4,5],[6,7,8]]
    return (type == 'row' ? sets[grid_number / 3] : sets[grid_number % 3] )
  end
